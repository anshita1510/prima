...
const client_1 = require("@prisma/client"); // Import AuthProvider
require("../../../types/express"); // Import express type extensions
const user_repository_1 = require("../../repository/auth/user.repository");
const inviteEmployee_usecase_1 = require("../../usecase/employees/inviteEmployee.usecase");
const setPassword_usecase_1 = require("../../usecase/password/setPassword.usecase");
const login_usecase_1 = require("../../usecase/auth/login.usecase");
const update_credentials_usecase_1 = require("../../usecase/auth/update.credentials.usecase");
const updatePassword_usecase_1 = require("../../usecase/auth/updatePassword.usecase");
const create_superAdmin_usecase_1 = require("../../usecase/super_admin/create.superAdmin.usecase");
...
const oauth_1 = require("../../../shared/utils/oauth");
/* -------------------------------------------------------------------------- */
/*                               INITIALIZATION                               */
/* -------------------------------------------------------------------------- */
...
const inviteEmployeeUsecase = new inviteEmployee_usecase_1.InviteEmployeeUsecase(userRepo, sendEmailUseCase);
/* -------------------------------------------------------------------------- */
...
class UserController {
    ...
    async createSuperAdmin(req, res) {
        try {
            const usecase = new create_superAdmin_usecase_1.CreateSuperAdminUsecase();
    ...
    async inviteEmployee(req, res) {
        try {
            ...
            const allowedDesignations = Object.values(client_1.Designation);
    ...
    async updatePassword(req, res) {
        try {
            ...
            const usecase = new updatePassword_usecase_1.UpdatePasswordUsecase(userRepo);
    ...
    async setPassword(req, res) {
        try {
            ...
            const usecase = new setPassword_usecase_1.SetPasswordUsecase(userRepo);
    ...
    async getAllUsers(req, res) {
        try {
            ...
            const andClauses = [];
            if (isSuperAdmin) {
                andClauses.push({ role: { not: client_1.Role.EMPLOYEE } });
            }
            if (search) {
                andClauses.push({
                    OR: [
    ...
    async googleLogin(req, res) {
        try {
            ...
            const googleUser = await oauth_1.OAuthService.verifyGoogleToken(token);
    ...
    async microsoftLogin(req, res) {
        try {
            ...
            let microsoftUser;
            try {
                microsoftUser = await oauth_1.OAuthService.verifyMicrosoftToken(microsoftToken);
            }
            catch (error) {
                // For development, we'll allow mock tokens
            ...
            if (!user) {
                ...
                user = await db_1.prisma.user.create({
                    data: {
                        ...
                        designation: "Employee",
                        role: client_1.Role.EMPLOYEE,
                        authProvider: client_1.AuthProvider.MICROSOFT,
                ...
                if (user.authProvider !== client_1.AuthProvider.MICROSOFT) {
    ...
    async updateCredentials(req, res) {
        try {
            ...
            const usecase = new update_credentials_usecase_1.UpdateCredentialsUsecase(userRepo);
    ...
    async deleteUser(req, res) {
        try {
            ...
            // Check if user exists
            const userToDelete = await db_1.prisma.user.findUnique({
                where: { id: userId },
                include: {
                    employee: true
                }
            ...
            if (userToDelete.role === client_1.Role.SUPER_ADMIN && currentUser.role !== client_1.Role.SUPER_ADMIN) {
            ...
            if (userToDelete.role === client_1.Role.SUPER_ADMIN) {
                const superAdminCount = await db_1.prisma.user.count({
    ...
    async resendInvitation(req, res) {
        try {
            ...
            const hashedTempPassword = await bcrypt_1.default.hash(tempPassword, 10);
    ...
    async downloadPostmanCollection(req, res) {
        try {
