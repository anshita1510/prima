...
class DashboardController {
     ...
     */
    async debugDatabaseContents(req, res) {
        try {
            const user = req.user;
            ...
            const allUsers = await db_1.prisma.user.findMany({
                select: {
                    id: true,
                    email: true,
                    firstName: true,
            ...
            // Get all companies
            const allCompanies = await db_1.prisma.company.findMany({
                select: {
                    id: true,
                    name: true,
                    code: true,
            ...
            const basicCounts = {
                totalUsersAll: await db_1.prisma.user.count(),
                totalUsersActive: await db_1.prisma.user.count({ where: { isActive: true } }),
                totalCompaniesAll: await db_1.prisma.company.count(),
                totalCompaniesActive: await db_1.prisma.company.count({ where: { isActive: true } }),
                totalAdminsAll: await db_1.prisma.user.count({
                    where: { role: { in: [client_1.Role.ADMIN, client_1.Role.SUPER_ADMIN] } }
                }),
                totalAdminsActive: await db_1.prisma.user.count({
                    where: {
    ...
    async getAdminDashboardStats(req, res) {
        try {
            ...
            const totalEmployees = await db_1.prisma.user.count({
                where: { companyId, isActive: true }
            });
            const totalTasks = await db_1.prisma.task.count({
                where: { project: { companyId } }
            });
            const completedTasks = await db_1.prisma.task.count({
                where: { project: { companyId }, status: 'COMPLETED' }
            });
            const overdueTasks = await db_1.prisma.task.count({
                where: { project: { companyId }, status: { notIn: ['COMPLETED', 'CANCELLED'] }, dueDate: { lt: new Date() } }
            });
            const pendingLeaves = await db_1.prisma.leave.count({
    ...
    async getDashboardStats(req, res) {
        try {
            ...
            const totalAdmins = await db_1.prisma.user.count({
                where: {
                    role: {
            ...
            const otherAdminLeaders = await db_1.prisma.user.count({
                where: {
                    role: { in: [client_1.Role.ADMIN, client_1.Role.SUPER_ADMIN] },
                    ceoId: null,
            ...
            const totalCompanies = await db_1.prisma.company.count();
            console.log('📊 Total Companies:', totalCompanies);
            const totalCeos = await db_1.prisma.user.count({
            ...
            const activeUsers = await db_1.prisma.user.count({
                where: {
                    updatedAt: {
                        gte: thirtyDaysAgo
                    }
            ...
            const pendingApprovals = await db_1.prisma.user.count({
                where: {
                    status: 'PENDING'
                }
            });
            ...
            const recentRegistrations = await db_1.prisma.user.count({
                where: {
            ...
            const rawRecentCompanies = await db_1.prisma.company.findMany({
                take: 5,
    ...
    async getRecentActivity(req, res) {
        try {
            ...
            // Get recent companies
            const recentCompanies = await db_1.prisma.company.findMany({
                take: 3,
                orderBy: {
                    createdAt: 'desc'
                },
    ...
    async getEmployeeDashboard(req, res) {
        try {
            ...
            const svc = new employee_dashboard_service_1.EmployeeDashboardService();
