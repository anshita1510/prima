...
class EmployeeController {
    constructor() {
        ...
        this.createEmployee = async (req, res) => {
            try {
                const { name, firstName, lastName, email, phone, designation, role, status = 'ACTIVE', departmentId, managerId, password, companyId } = req.body;
                const userContext = req.user;
                if (!userContext) {
                    return res.status(401).json({ success: false, message: 'Authentication required' });
                }
                // ✅ Support both name formats from Flutter and web
                const resolvedFirstName = firstName?.trim() || name?.trim().split(' ')[0] || '';
                const resolvedLastName = lastName?.trim() || name?.trim().split(' ').slice(1).join(' ') || resolvedFirstName;
                const resolvedName = name?.trim() || `${resolvedFirstName} ${resolvedLastName}`.trim();
                ...
                const result = await prisma.$transaction(async (tx) => {
                    const user = await tx.user.create({
                        data: {
                            ...
                            role: client_1.Role[role.toUpperCase()] || client_1.Role.EMPLOYEE,
                            status: status.toUpperCase() || client_1.Status.ACTIVE,
        ...
        this.updateEmployee = async (req, res) => {
            try {
                const { employeeId } = req.params;
                const { name, email, phone, designation, role, status, location, departmentId, managerId } = req.body;
                const userContext = req.user;
                if (!userContext) {
                    return res.status(401).json({
                        success: false,
        ...
        this.getManagerTeamMembers = async (req, res) => {
            try {
                const userContext = req.user;
                if (!userContext) {
                    return res.status(401).json({
        ...
        this.getUnassignedEmployees = async (req, res) => {
            try {
                const userContext = req.user;
                if (!userContext) {
                    return res.status(401).json({
        ...
        this.assignEmployeeToManager = async (req, res) => {
            try {
                const { employeeId } = req.params;
                const userContext = req.user;
                if (!userContext) {
