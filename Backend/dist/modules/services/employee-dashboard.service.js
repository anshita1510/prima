...
class EmployeeDashboardService {
    async getEmployeeDashboard(employeeId, companyId) {
        const now = new Date();
        const weekStart = startOfWeekMonday(now);
        const weekEnd = addDays(weekStart, 7);
        const assignedTasks = await db_1.prisma.task.findMany({
            where: {
        ...
        const activeAssigned = assignedTasks.filter((t) => t.status !== client_1.TaskStatus.CANCELLED);
        const myTasks = activeAssigned.length;
        let completed = 0;
        let inProgress = 0;
        let overdue = 0;
        for (const t of activeAssigned) {
            if (t.status === client_1.TaskStatus.COMPLETED)
                completed += 1;
            else if (t.status === client_1.TaskStatus.IN_PROGRESS || t.status === client_1.TaskStatus.IN_REVIEW)
                inProgress += 1;
            if (t.dueDate &&
                new Date(t.dueDate) < now &&
                t.status !== client_1.TaskStatus.COMPLETED &&
                t.status !== client_1.TaskStatus.CANCELLED) {
                overdue += 1;
            }
        }
        const completionRate = myTasks > 0 ? Math.round((completed / myTasks) * 1000) / 10 : 0;
        const attendanceRows = await db_1.prisma.attendance.findMany({
            where: {
                employeeId,
                companyId,
        ...
        const timeEntriesWeek = await db_1.prisma.taskTimeEntry.findMany({
            where: {
                employeeId,
        ...
        for (let m = 5; m >= 0; m--) {
            ...
            const [tasksDone, timeAgg] = await Promise.all([
                ...
                db_1.prisma.taskTimeEntry.aggregate({
                    where: {
                        employeeId,
        ...
        const recentDone = await db_1.prisma.task.findMany({
            where: {
                assignedToId: employeeId,
                project: { companyId },
                status: client_1.TaskStatus.COMPLETED,
