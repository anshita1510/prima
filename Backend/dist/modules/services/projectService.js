...
class ProjectService {
    ...
    async createProject(data, req) {
        try {
            ...
            await this.assignTeamMember({
                ...
                role: client_1.ProjectRole.MANAGER
            }, req);
    ...
    async updateProject(projectId, data, req) {
        try {
    ...
    async getAllProjects(companyId, departmentId, ownerId) {
        try {
            const whereClause = {
                isActive: true
            };
            ...
            const projectsWithStats = projects.map((project) => {
                ...
                const completedTasks = project.tasks.filter((task) => task.status === client_1.TaskStatus.COMPLETED).length;
    ...
    async assignTeamMember(data, req) {
        try {
    ...
    async removeTeamMember(projectId, employeeId, req) {
        try {
    ...
    async updateTeamMemberRole(projectId, employeeId, role, req) {
        try {
            const projectMember = await prisma.projectMember.update({
                where: {
                    projectId_employeeId: {
    ...
    async updateTask(taskId, data, req) {
        try {
            ...
            if (data.status === client_1.TaskStatus.COMPLETED && !data.completedAt) {
    ...
    async getProjectTasks(projectId, status, assignedToId) {
        try {
            const whereClause = {
                projectId,
                isActive: true
    ...
    async getProjectDashboardStats(companyId, departmentId) {
        try {
            const whereClause = {
                isActive: true
            };
            ...
            if (departmentId)
                whereClause.departmentId = departmentId;
            const [totalProjects, activeProjects, completedProjects, onHoldProjects, totalTasks, completedTasks, overdueTasks] = await Promise.all([
                prisma.project.count({ where: whereClause }),
                prisma.project.count({ where: { ...whereClause, status: client_1.ProjectStatus.ACTIVE } }),
                prisma.project.count({ where: { ...whereClause, status: client_1.ProjectStatus.COMPLETED } }),
                prisma.project.count({ where: { ...whereClause, status: client_1.ProjectStatus.ON_HOLD } }),
                prisma.task.count({
                    where: {
                        project: whereClause,
                        isActive: true
