...
class TaskService {
    constructor(prisma) {
        this.prisma = prisma;
        this.taskRepository = new task_repository_1.TaskRepository(prisma);
    }
    async createTask(data, createdById, companyId, role) {
    ...
    async getTasks(query, companyId, employeeId, isManager) {
    ...
    async getTaskById(id, companyId, employeeId, isManager) {
    ...
    async updateTask(id, data, companyId, employeeId, isManager) {
        const task = await this.taskRepository.findById(id, companyId);
        if (!task) {
            throw new Error('Task not found');
        }
    ...
    async deleteTask(id, companyId, employeeId, isManager) {
        const task = await this.taskRepository.findById(id, companyId);
        if (!task) {
            throw new Error('Task not found');
        }
    ...
    async addTaskComment(taskId, data, authorId, companyId) {
    ...
    async getTaskStats(companyId, employeeId, isManager) {
        const where = {
            isActive: true
        };
