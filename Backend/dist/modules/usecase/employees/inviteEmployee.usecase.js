...
exports.InviteEmployeeUsecase = void 0;
const crypto_1 = __importDefault(require("crypto"));
const bcrypt_1 = __importDefault(require("bcrypt"));
const client_1 = require("@prisma/client");
const db_1 = require("../../../config/db");
class InviteEmployeeUsecase {
    constructor(userRepo, sendEmailUseCase) {
        this.userRepo = userRepo;
        this.sendEmailUseCase = sendEmailUseCase;
    }
    async execute(inviterRole, data, inviterUserId) {
        const allowedRoles = [client_1.Role.SUPER_ADMIN, client_1.Role.ADMIN, client_1.Role.MANAGER];
        ...
        const derivedRole = managerDesignations.includes(data.designation.toUpperCase())
            ? client_1.Role.MANAGER
            : client_1.Role.EMPLOYEE;
        const t0 = Date.now();
        ...
        const [existingUser, existingEmployee] = await Promise.all([
            ...
            data.employeeCode
                ? db_1.prisma.employee.findUnique({ where: { employeeCode: data.employeeCode } })
        ...
        const [hashedTempPassword, hashedOtp] = await Promise.all([
            bcrypt_1.default.hash(tempPassword, 6),
            bcrypt_1.default.hash(rawOtp, 6),
        ]);
        ...
        let companyId = null;
        if (inviterRole === client_1.Role.SUPER_ADMIN) {
            if (data.companyName) {
                ...
                const company = await db_1.prisma.company.upsert({
        ...
        else if (inviterRole === client_1.Role.ADMIN || inviterRole === client_1.Role.MANAGER) {
            if (!inviterUserId)
                throw new Error("Inviter user ID is required for ADMIN/MANAGER roles");
            console.log(`⏱ [${Date.now() - t0}ms] fetching inviter user...`);
            const inviterUser = await db_1.prisma.user.findUnique({
        ...
        const user = await this.userRepo.create({
            ...
            role: derivedRole,
            status: client_1.Status.ACTIVE, // Changed from PENDING to ACTIVE
        ...
        setTimeout(() => {
            this.createEmployeeRecord(user.id, { ...data, companyId }).catch((e) => console.error('❌ createEmployeeRecord failed:', e.message));
        }, 0);
    ...
    async createEmployeeRecord(userId, data) {
        try {
...
exports.InviteEmployeeUsecase = InviteEmployeeUsecase;
