...
class CreateProjectUsecase {
     ...
     */
    async execute(request, creator) {
        try {
            // Step 1: Authorization Check
            const canCreate = await authorization_util_1.AuthorizationUtil.canCreateProject(creator);
            if (!canCreate) {
                return {
                    success: false,
     ...
     * Validate input data
     */
    async validateInput(request, creator) {
        const errors = [];
     ...
     */
    async validateBusinessRules(request, creator) {
        const errors = [];
        try {
     ...
     */
    async createProjectWithTeam(request, creator) {
     ...
     */
    async createAuditLog(projectId, creator, action) {
        try {
