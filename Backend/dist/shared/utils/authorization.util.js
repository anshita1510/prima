...
class AuthorizationUtil {
     ...
     */
    static async hasPermission(action, user, resource) {
        const permissionRule = permissions_config_1.ALL_PERMISSIONS[action];
        if (!permissionRule) {
        ...
        if (permissionRule.customConditions) {
            if (!resource) {
                ...
                const nonResourceConditions = permissionRule.customConditions.filter(condition => ['isActiveUser', 'hasCompanyAccess'].includes(condition));
                if (nonResourceConditions.length > 0) {
                    return await this.checkCustomConditions(nonResourceConditions, user, {});
                }
            }
            else {
                return await this.checkCustomConditions(permissionRule.customConditions, user, resource);
            }
        }
        return false;
    }
     ...
     */
    static hasRolePermission(userRole, allowedRoles) {
        // Direct role match
        if (allowedRoles.includes(userRole)) {
            return true;
        }
        // Hierarchy check - higher roles inherit lower role permissions
        const userRoleLevel = permissions_config_1.ROLE_HIERARCHY[userRole];
        return allowedRoles.some(role => {
            const requiredLevel = permissions_config_1.ROLE_HIERARCHY[role];
     ...
     */
    static hasDesignationPermission(userDesignation, allowedDesignations) {
        // Direct designation match
        if (allowedDesignations.includes(userDesignation)) {
            return true;
        }
        // Hierarchy check
        const userDesignationLevel = permissions_config_1.DESIGNATION_HIERARCHY[userDesignation];
        return allowedDesignations.some(designation => {
            const requiredLevel = permissions_config_1.DESIGNATION_HIERARCHY[designation];
     ...
     * Check custom conditions
     */
    static async checkCustomConditions(conditions, user, resource) {
     ...
     */
    static async evaluateCondition(condition, user, resource) {
        switch (condition) {
            case 'isActiveUser':
            ...
            case 'hasCompanyAccess':
                ...
                if (!resource || !resource.projectId)
                    return true; // Skip if no project context
                return await this.hasCompanyAccess(user.companyId, resource.projectId);
            case 'isProjectOwner':
                if (!resource.projectId || !user.employeeId)
                    return false;
            ...
            case 'isTaskAssignee':
                if (!resource.taskId || !user.employeeId)
                    return false;
     ...
     */
    static async hasCompanyAccess(userCompanyId, projectId) {
        try {
            if (!projectId)
    ...
    static async isProjectOwner(projectId, employeeId) {
        try {
            const project = await prisma.project.findUnique({
                where: { id: projectId },
                select: { ownerId: true }
    ...
    static async isProjectManager(projectId, employeeId) {
        try {
    ...
    static async isProjectMember(projectId, employeeId) {
        try {
            const projectMember = await prisma.projectMember.findUnique({
                where: {
                    projectId_employeeId: {
    ...
    static async isTaskAssignee(taskId, employeeId) {
        try {
    ...
    static async isTaskCreator(taskId, employeeId) {
        try {
            const task = await prisma.task.findUnique({
                where: { id: taskId },
     ...
     */
    static async getUserPermissions(user, resource) {
        const permissions = [];
        for (const [action] of Object.entries(permissions_config_1.ALL_PERMISSIONS)) {
     ...
     */
    static async hasAnyPermission(actions, user, resource) {
     ...
     */
    static async hasAllPermissions(actions, user, resource) {
        for (const action of actions) {
            if (!(await this.hasPermission(action, user, resource))) {
                return false;
            }
