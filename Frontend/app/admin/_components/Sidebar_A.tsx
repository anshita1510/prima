...
type NavItem = {
  ...
  icon: LucideIcon;
  match?: MatchMode;
  /** Optional count badge; omit when no data (no placeholder counts). */
  badge?: { count: number; tone: "primary" | "warn" };
};
...
const NAV_SECTIONS: NavSection[] = [
  ...
  },
  {
    title: "People",
    items: [
      { id: "team", name: "Team", icon: Users, href: "/enhanced-tms/team" },
      { id: "attendance", name: "Attendance", icon: UserCheck, href: "/admin/attendance" },
      { id: "performance", name: "Performance", icon: TrendingUp, href: "/admin/performance" },
      { id: "timesheets", name: "Timesheets", icon: Clock, href: "/admin/timesheets" },
  ...
  },
  {
    ...
    items: [
      ...
      { id: "calendar", name: "Calendar", icon: Calendar, href: "/enhanced-tms/calendar" },
      { id: "meetings", name: "Meetings", icon: Video, href: "/admin/meetings" },
    ],
  },
  {
    title: "Approvals",
    items: [
      { id: "approvals", name: "Approvals", icon: ClipboardList, href: "/admin/leave-approvals" },
      { id: "leave", name: "Leave Management", icon: CalendarDays, href: "/admin/leave-management" },
    ],
  },
];
...
export default function Sidebar() {
  ...
  /** Apply persisted width before paint so layout doesn’t jump. */
  useLayoutEffect(() => {
    try {
      ...
      document.documentElement.style.setProperty("--sidebar-width", `${clampSidebarWidth(parsed)}px`);
    } catch {
      /* ignore */
    }
  }, []);
  ...
  const startSidebarResize = useCallback((e: React.MouseEvent) => {
    ...
    const up = () => {
      ...
      dragCleanupRef.current = null;
      try {
        const finalW = readSidebarWidthFromCss();
        localStorage.setItem(SIDEBAR_WIDTH_STORAGE_KEY, String(clampSidebarWidth(finalW)));
      } catch {
        /* ignore */
      }
    };
  ...
  const linkBody = (item: NavItem, active: boolean) => (
    <>
      ...
      {item.badge != null && item.badge.count > 0 && (
        <span
          className={`relative z-[1] flex h-5 min-w-[1.25rem] shrink-0 items-center justify-center rounded-full px-1.5 text-[10px] font-bold tabular-nums ${
            item.badge.tone === "warn" ? "sidebar-app__badge--warn" : "sidebar-app__badge--primary"
          }`}
        >
  ...
  return (
    <>
      ...
      >
        <div className="sidebar-app__header flex h-[73px] shrink-0 items-center justify-between border-b px-4">
        ...
        <nav className="min-h-0 flex-1 space-y-0 overflow-y-auto overflow-x-hidden px-2 py-3">
          {NAV_SECTIONS.map((section, si) => (
            <div key={section.title} className={si > 0 ? "sidebar-app__section-divider mt-5 border-t pt-5" : ""}>
        ...
        {/* Desktop: drag right edge to change --sidebar-width (persisted). Double-click to reset. */}
        <div
          role="separator"
