...
function downloadUsersCsv(users: User[]) {
  ...
  const row = (u: User) => headers.map((h: CsvKey) => escape(String(u[h] ?? ''))).join(',');
...
export default function CreateUserPage() {
  ...
  const scrollToInvite = useCallback(() => {
    document.getElementById('admin-invite-panel')?.scrollIntoView({ behavior: 'smooth', block: 'start' });
  }, []);
    ...
    <div className="user-mgmt-console flex min-h-0 w-full flex-1 flex-col gap-6">
      <header className="um-page-header">
        ...
        <div className="um-header-actions">
          ...
          <button type="button" className="um-btn-header-solid" onClick={scrollToInvite}>
      ...
      <div className="min-h-0 flex-1 overflow-y-auto overscroll-contain">
        <UserManagementCreateWorkspace refreshKey={statsRefresh}>
